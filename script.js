...
const pathways = {
  mindfulness: {
    ...
    emailSubject: "Week 1: Start with one steady breath",
    emailIntro:
      "Welcome to Presence & Mindfulness. This week is about building a relationship to attention that feels supportive, not performative.",
    plan: [
      {
        title: "Three-minute practice",
        copy: "Begin each morning with one guided breathing pause and one sentence of intention."
      },
      {
        title: "Suggested reading",
        copy: "Read a short Tricycle piece on attention, distraction, and the dignity of starting small."
      },
      {
        title: "Journal prompt",
        copy: "Ask: What changes when I stop trying to fix the moment and simply stay with it?"
      }
    ]
  },
  compassion: {
    title: "Compassion & Relationships",
    summary:
    ...
    emailSubject: "Week 1: Begin with compassion close to home",
    emailIntro:
      "Welcome to Compassion & Relationships. This week introduces Buddhist practice through care, conversation, and the small choices that reshape how we meet others.",
    plan: [
      {
        title: "Five-minute practice",
        copy: "Try a loving-kindness meditation focused first on yourself, then on one person you care about."
      },
      {
        title: "Suggested reading",
        copy: "Read a Tricycle feature on compassion as a discipline of attention, not just a feeling."
      },
      {
        title: "Journal prompt",
        copy: "Ask: Where do I confuse self-protection with disconnection, and what would gentle honesty look like instead?"
      }
    ]
  },
  interconnection: {
    title: "Interconnection & Nature",
    summary:
  ...
  creativity: {
    ...
    emailSubject: "Week 1: Let attention become expression",
    emailIntro:
      "Welcome to Creativity & Imagination. This week uses making, noticing, and artistic play to open Buddhist themes without flattening them into content alone.",
    plan: [
      {
        title: "Creative practice",
        copy: "Set a timer for eight minutes and make something small while noticing breath, impulse, and texture."
      },
      {
        title: "Suggested reading",
        copy: "Read a Tricycle piece on contemplative art, symbol, and why beauty can be a carrier of spiritual meaning."
      },
      {
        title: "Journal prompt",
        copy: "Ask: What part of me becomes audible when I stop trying to be productive and start paying attention?"
      }
    ]
  },
  foundations: {
...
const questions = [
  {
    ...
    visualCopy:
      "Use a quiet interior, a still portrait, or a restrained scene that suggests overwhelm giving way to steadiness.",
    options: [
      { label: "Sitting quietly and focusing on my breath", path: "mindfulness", signal: "quiet focus" },
      { label: "Talking with someone and feeling understood", path: "compassion", signal: "care and understanding" },
      { label: "Going outside or being in nature", path: "interconnection", signal: "nature and spaciousness" },
      { label: "Making art, writing, or creating something", path: "creativity", signal: "creative expression" },
      { label: "Reading or learning to understand it better", path: "foundations", signal: "clear understanding" }
    ]
  },
  {
    ...
    visualCopy:
      "Use an image with emotional atmosphere: a hand, a room, a horizon, or an object that suggests longing or intention.",
    options: [
      { label: "I want to feel calmer", path: "mindfulness", signal: "calm" },
      { label: "I want to become kinder", path: "compassion", signal: "kindness" },
      { label: "I want to feel more connected", path: "interconnection", signal: "connection" },
      { label: "I want inspiration", path: "creativity", signal: "inspiration" },
      { label: "I want deeper understanding", path: "foundations", signal: "deeper understanding" }
    ]
  },
  {
    ...
    visualCopy:
      "Use a study table, conversation setting, notebook, or contemplative scene that can hold different modes of learning.",
    options: [
      { label: "Through daily practice", path: "mindfulness", signal: "daily rhythm" },
      { label: "Through conversation", path: "compassion", signal: "conversation" },
      { label: "Through lived experience", path: "interconnection", signal: "lived experience" },
      { label: "Through creative exploration", path: "creativity", signal: "creative exploration" },
      { label: "Through structured study", path: "foundations", signal: "structured learning" }
    ]
  },
  {
    ...
    visualCopy:
      "Use a more contemplative image here: path, threshold, window light, or a single object with enough tension to feel searching.",
    options: [
      { label: "How can I be more present?", path: "mindfulness", signal: "being present" },
      { label: "How can I reduce suffering in relationships?", path: "compassion", signal: "relationship suffering" },
      { label: "How can I understand my place in the world?", path: "interconnection", signal: "place in the world" },
      { label: "How can creativity be spiritual?", path: "creativity", signal: "spiritual creativity" },
      { label: "What does Buddhism actually teach?", path: "foundations", signal: "what Buddhism teaches" }
    ]
  },
  {
    ...
    visualCopy:
      "Use an image tied to practice: a cushion, walking path, paper and pencil, or a reading surface with open space.",
    options: [
      { label: "Guided meditation", path: "mindfulness", signal: "guided meditation" },
      { label: "Loving-kindness practice", path: "compassion", signal: "loving-kindness" },
      { label: "Walking meditation", path: "interconnection", signal: "walking meditation" },
      { label: "Mindful drawing or journaling", path: "creativity", signal: "mindful making" },
      { label: "A talk on the Four Noble Truths", path: "foundations", signal: "foundational teaching" }
    ]
  },
  {
    ...
    visualCopy:
      "Use a concluding image that feels distilled and spacious: one object, one gesture, or one environment with visual quiet.",
    options: [
      { label: "Awareness", path: "mindfulness", signal: "awareness" },
      { label: "Compassion", path: "compassion", signal: "compassion" },
      { label: "Belonging", path: "interconnection", signal: "belonging" },
      { label: "Expression", path: "creativity", signal: "expression" },
      { label: "Wisdom", path: "foundations", signal: "wisdom" }
    ]
  }
];
...
const questionMediaSlot = document.getElementById("question-media-slot");
const questionVisualLabel = document.getElementById("question-visual-label");
...
const emailVisualTitle = document.getElementById("email-visual-title");
const emailVisualCopy = document.getElementById("email-visual-copy");
...
function setMediaImage(element, imagePath) {
  ...
  if (!imagePath) {
    ...
    element.classList.remove("media-slot-has-image", "mini-visual-has-image");
    return;
  }

  element.style.backgroundImage = `url('${imagePath}')`;
  if (element.classList.contains("media-slot")) {
    element.classList.add("media-slot-has-image");
  }
...
function renderQuestion() {
  ...
  questionKicker.textContent = `Question ${state.currentQuestion + 1}`;
  questionPrompt.textContent = question.prompt;
  questionNote.textContent = question.note;
  questionVisualLabel.textContent = question.visualLabel || "Image Placeholder";
  questionVisualTitle.textContent = question.visualTitle || "Question editorial image";
  questionVisualCopy.textContent =
    question.visualCopy ||
    "Replace this area with an image, illustration, or quiet still-life that supports the emotional tone of the question.";
  setMediaImage(questionMediaSlot, question.image);
  ...
  nextButton.textContent =
    state.currentQuestion === questions.length - 1 ? "See My Pathway" : "Next";
  ...
  question.options.forEach((option, optionIndex) => {
    ...
    button.style.setProperty("--item-index", optionIndex);
    button.innerHTML = `
      <span class="answer-badge">${String.fromCharCode(65 + optionIndex)}</span>
...
function computeScores() {
  ...
  state.answers.forEach((answerIndex, questionIndex) => {
    ...
    const bonus = questions[questionIndex].priority ? 0.35 : 0;
    scores[option.path] += 1 + bonus;
  });

  return scores;
}

function determinePathway(scores) {
  const sorted = Object.entries(scores).sort((left, right) => right[1] - left[1]);
  const topScore = sorted[0][1];
  const tied = sorted.filter((entry) => entry[1] === topScore).map((entry) => entry[0]);

  if (tied.length === 1) {
    return tied[0];
  }

  const priorityOrder = [1, 5, 3, 0, 4, 2];
  for (const questionIndex of priorityOrder) {
    const answerIndex = state.answers[questionIndex];
    if (answerIndex === null) {
      continue;
    }
    const candidate = questions[questionIndex].options[answerIndex].path;
...
function renderEmailPreview() {
  ...
  emailVisualTitle.textContent = pathway.emailVisualTitle;
  emailVisualCopy.textContent = pathway.emailVisualCopy;
  practiceCopy.textContent = pathway.plan[0].copy;
  readingCopy.textContent = pathway.plan[1].copy;
...
function prefillAnswersForPath(pathKey) {
...
function applyPreviewFromUrl() {
  ...
  const questionParam = Number.parseInt(params.get("q") || "1", 10);

  if (screen === "quiz") {
    const questionIndex = Number.isNaN(questionParam)
      ? 0
      : Math.min(Math.max(questionParam - 1, 0), questions.length - 1);
